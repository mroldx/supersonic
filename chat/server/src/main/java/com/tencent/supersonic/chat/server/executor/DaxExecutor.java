...
public class DaxExecutor implements ChatQueryExecutor {
    ...
    public QueryResult execute(ExecuteContext executeContext) {
        ...
        if (queryResult != null) {
            ...
            if (queryResult.getQueryState().equals(QueryState.SUCCESS)
                ...
                Text2DAXExemplar exemplar =
                        JsonUtil.toObject(
                                JsonUtil.toString(executeContext.getParseInfo().getProperties()
                                        .get(Text2DAXExemplar.PROPERTY_KEY)),
                                Text2DAXExemplar.class);
    ...
    private QueryResult doExecute(ExecuteContext executeContext) {
        ...
        QueryDaxReq sqlReq =
                QueryDaxReq.builder().sql(parseInfo.getSqlInfo().getCorrectedS2SQL()).build();
